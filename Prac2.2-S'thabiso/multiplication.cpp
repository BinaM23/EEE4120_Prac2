...
int main(void)
{
	...
	clFinish(queue);
	end2 = clock();
	float execTime = (((float) end2 - start2)/CLOCKS_PER_SEC); //useful execution time
	
	
	//OVERHEAD STOPS HERE 
	end=clock();
	float TotalTime = ((float) end - start)/CLOCKS_PER_SEC; //overhead + useful time
	float overheadTime=TotalTime-execTime; //calculate overhead time alone
	...
	//Print times
	printf("execution time of Kernel: %0.8f sec \n", execTime); //prints and adds times to .txt
