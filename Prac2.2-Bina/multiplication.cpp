...
void createRandomSquareMatrix(int Size, int* squareMatrix, bool displayMatrices){
	srand(time(0)); //ensures random numbers generated are different each time the code is run 
...
int main(void)
{
	...
	int Size = 3; //can be changed
	string string_Size = to_string(Size); //size as a string
	int countA = Size*Size;
	int matrixA[countA];
	createKnownSquareMatrix(Size,matrixA,displayMatrices);
	//createRandomSquareMatrix(Size,matrixA,displayMatrices); //later for testing random data sets, correlation stuff
	...
	//------------------------------------------------------------------------
	 ...
	 //OVERHEAD STARTS HERE so overhead timer started (parallel set up, memory, kernels, building programs, contexts and queing)
	 start=clock();
	...
	cl_kernel kernel = clCreateKernel(program, "matrixMultiplication", &err); //kernel of interest
	...
	clSetKernelArg(kernel, 0, sizeof(cl_mem), &matrixA_buffer); //creates pointers for kernels to access buffers
	...
	cl_int err4 = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &global_size, &local_size, 0, NULL, NULL); //let kernels run
	
	

	printf("\nKernel check: %i \n",err4);

	clock_t start2, end2; //timers for critical section
	...
	clFinish(queue);
	//INITIAL RUN (CACHE WARMING) so not measured

	//Start measuring execution time of critical section

	/*int No_of_runs = 10; //loop runs kernel No_of_runs times if no loop in python script
	start2 = clock(); //start second timer
	...
	clFinish(queue);
	end2 = clock();
	float meanTime = (((float) end2 - start2)/CLOCKS_PER_SEC)/No_of_runs;
	*/
	//not sure if result is after the 10 runs, we can't see result of an individual run

	//runs and measures time once, this repeats 10 times via OCLscript.py 
	start2 = clock();
	clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &global_size, &local_size, 0, NULL, NULL);
	clFinish(queue);
	end2 = clock();
	float meanTime = (((float) end2 - start2)/CLOCKS_PER_SEC); //not actually mean time but kept names consistent for easy math
	...
	float overheadTime=TotalTime-meanTime;


	//writing results of multiplication and times to .txt file
	string filename("OCLoutput_" +  string_Size + "x" + string_Size + ".txt");
	ofstream myfile;
	...
	printf(" mean execution time of Kernel: %0.8f sec \n", meanTime); //prints and adds times to .txt
	printf("Overhead time (excl kernel): %0.8f sec \n", overheadTime);
	myfile << "Run Time: " << meanTime << " sec \n"; 
